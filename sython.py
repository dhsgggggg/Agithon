...
@sython.on(events.NewMessage(outgoing=True, pattern=r"\.م10"))
async def _(event):
    start = datetime.datetime.now()
    await event.edit("جاري التحميل...")
    end = datetime.datetime.now()
    ...
    await event.edit(f'''
父 مطورين السورس |
‎⿻┊‌‎‌‎المطور الرسمي 𖠄 @D_2_B_2
‌‎⿻┊‌‎‌‎المالك الرسمي 𖠄 @T_Y_V ٫
''')
...
async def update(event):
    await event.edit("**⚘︙ جـاري تـحديث اجيثو العـرب (انتظر)
